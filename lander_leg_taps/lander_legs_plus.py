...
RESULTS_OUTPUT_DIR = "/home/logan/gym/gymjam/lander_legs_plus_stats/"
...
lander_leg_taps_list=np.zeros(100050)
lander_leg_taps_list.fill(0)
max_lander_leg_taps=0
...
current_run_id=-1
current_video_id=1
successful_fitness_list=np.zeros(100)
# successful_sequence_list=np.zeros(100)
...
class GameEvaluator:
    ...
    def run(self, agent, render=False):
        ...
        lander_leg_taps=0
        left_leg_ground_contact=False
        right_leg_ground_contact=False
        
        global max_lander_contacts_difference
        global lander_steps_differences_list
        global max_lander_leg_taps
        global lander_leg_taps_list
        global current_run
        ...
        if render:
            # env.render()
            rec=VideoRecorder(env,path='/home/logan/gym/gymjam/videoResults_legs_plus/temp'+str(current_run_id)+'_'+str(current_video_id)+'.mp4')
        ...
        while not done:
            ...
            if(observation[6]==1.0 and left_leg_ground_contact==False):
                left_leg_ground_contact=True
                lander_leg_taps+=1

            if(observation[6]==0.0 and left_leg_ground_contact==True):
                left_leg_ground_contact=False

            if(observation[7]==1.0 and right_leg_ground_contact==False):
                right_leg_ground_contact=True
                lander_leg_taps+=1

            if(observation[7]==0.0 and right_leg_ground_contact==True):
                right_leg_ground_contact=False
        ...
        if(lander_leg_taps>max_lander_leg_taps):
            max_lander_leg_taps=lander_leg_taps
            print('new max leg taps is ',max_lander_leg_taps)
        ...
        lander_leg_taps_list[current_run]=lander_leg_taps
        # time.sleep(50)
        ...
        if(current_run%1000==0):
            ...
            np.savetxt("lander_leg_taps_list"+current_run_id+".csv",lander_leg_taps_list,delimiter=",")
        ...
        # For experiment endpointBC and others
            ...
            if(lander_contacts_difference==-80):
                lander_contacts_difference=101.0
            agent.features=(lander_contacts_difference,lander_leg_taps)
...
def runME(run_id, game, sequence_len,
    ...
    # For experiment endpointBC and others
        ...
        feature_ranges = [(0.0, 101.0), (0.0, 100.0)]
