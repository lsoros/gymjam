...
class GameEvaluator:
    ...
    def run(self, agent, render=False):
        ...
        if(render):
            print('recording path is ',rec.path)
            rec.close()
...
def runME(run_id, game, sequence_len,
    ...
    for individuals_evaluated in range(num_individuals):
        ...
        if cur_agent.fitness >= 200:
            ...
            # commands_length = len(cur_agent.commands)
            successful_sequence_string = ""
            for sequence_data in cur_agent.commands:
                successful_sequence_string += str(sequence_data)
            successful_sequence_list[current_video_id] = successful_sequence_string
            print('its sequence is ', successful_sequence_string)
    ...
    np.savetxt("successful_fitnesses"+str(current_run_id)+".csv",successful_fitness_list, delimiter=",")
    np.savetxt("successful_sequences"+str(current_run_id)+".csv",successful_sequence_list, delimiter=",")
