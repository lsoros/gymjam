...
RESULTS_OUTPUT_DIR = "/Users/bharathsurianarayanan/Desktop/gymjam/lander_angle_stats/"
...
ME_LANDER_STEPS_BC='ME_stepsdiffBC'
ME_LANDER_ANGLE_VELOCITY_BC='ME-angleVelocityBC'
MODES = [ME_ENDPOINT_BC, ME_POLYHASH_BC, ME_FITNESS_BC, ME_ENTROPY_BC]

lander_finish_angle_list=np.zeros(100050)
lander_finish_velocity_list=np.zeros(100050)
lander_finish_angle_list.fill(-1000)
lander_finish_velocity_list.fill(-1000)
min_lander_contact_angle=0
max_lander_contact_angle=0
max_lander_contact_resultant_velocity=-10000
min_lander_contact_resultant_velocity=10000

# lander_steps_differences_list=np.zeros(100050)
...
current_run_id=-1
current_video_id=1
successful_fitness_list=np.zeros(1000)
successful_sequence_list=np.zeros(1000)
...
class GameEvaluator:
    ...
    def run(self, agent, render=False):
        ...
        lander_contact_angle=-1000
        lander_contact_resultant_velocity=-1000
        initial_ground_contact=False
        global max_lander_contact_angle
        global min_lander_contact_angle
        global lander_finish_angle_list
        global lander_finish_velocity_list
        global max_lander_contact_resultant_velocity
        global current_run
        global current_run_id
        global current_video_id
        global min_lander_contact_resultant_velocity

        if render:
            # env.render()
            rec=VideoRecorder(env,path='/Users/bharathsurianarayanan/Desktop/gymjam/lander_angle_stats/'+str(current_run_id)+'_'+str(current_video_id)+'.mp4')
        ...
        while not done:
            ...
            if((observation[6]==1.0 or observation[7]==1.0) and initial_ground_contact == False):
                initial_ground_contact = True
                lander_contact_angle = observation[4]
                lander_contact_velocityX=observation[2]
                lander_contact_velocityY=observation[3]
                lander_contact_resultant_velocity=math.sqrt((lander_contact_velocityX**2)+(lander_contact_velocityY**2))
                # print('resultant_velocity is ',lander_contact_resultant_velocity)
        ...
        # if(lander_contact_resultant_velocity>max_lander_contact_resultant_velocity):
        #     max_lander_contact_resultant_velocity=lander_contact_resultant_velocity
        #     print("new max resultant_velocity is ", max_lander_contact_resultant_velocity)

        # if(lander_contact_resultant_velocity<min_lander_contact_resultant_velocity):
        #     if(lander_contact_resultant_velocity!=-1000):
        #         min_lander_contact_resultant_velocity=lander_contact_resultant_velocity
        #         print("new min resultant_velocity is ", min_lander_contact_resultant_velocity)

        # if(lander_contact_angle>max_lander_contact_angle):
        #     max_lander_contact_angle=lander_contact_angle
        #     print("new max angle is ", max_lander_contact_angle)

        # if(lander_contact_angle<min_lander_contact_angle and lander_contact_angle!=-1000):
        #     min_lander_contact_angle=lander_contact_angle
        #     print("new min angle is ", min_lander_contact_angle)

        # if(lander_contacts_difference!=-80):
        #     print('time to touch the ground',lander_contacts_difference)
        if( lander_contact_resultant_velocity!=-1000 and lander_contact_resultant_velocity<0):
            print('velocity is negative')
            sys.exit(0)
        
        lander_finish_angle_list[current_run]=lander_contact_angle
        lander_finish_velocity_list[current_run]=lander_contact_resultant_velocity
        ...
        if(current_run%1000==0):
            ...
            print('current velocity is',lander_contact_resultant_velocity)
            np.savetxt("/Users/bharathsurianarayanan/Desktop/gymjam/lander_angle_stats/lander_contact_angle_list"+current_run_id+".csv",lander_finish_angle_list,delimiter=",")
            np.savetxt("/Users/bharathsurianarayanan/Desktop/gymjam/lander_angle_stats/lander_contact_resultant_velocity_list"+current_run_id+".csv",lander_finish_velocity_list,delimiter=",")
            # np.savetxt("lan")
        ...
        elif self.mode == ME_LANDER_ANGLE_VELOCITY_BC:
...
def runME(run_id, game, sequence_len,
    ...
    global current_run_id
    global current_video_id
    global successful_fitness_list
    global successful_sequence_list
    ...
    # For experiment entropyBC
        ...
        feature_ranges = [(0.0, sequence_len), (0.0, sequence_len)]
    # For experiment endpointBC and others
    elif mode == ME_LANDER_ANGLE_VELOCITY_BC:
        # print('using ME_LANDER_ANGLE_VELOCITY_BC')
        feature_ranges = [(0, 30), (-25.0, 25.1)]
    ...
    for individuals_evaluated in range(num_individuals):
        ...
        if cur_agent.fitness >= 200:
            print('success: ', cur_agent.fitness, cur_agent.action_count)
            successful_fitness_list[current_video_id] = cur_agent.fitness
            # successful_sequence_list[current_video_id] = cur_agent.commands
            print('its sequence is ', cur_agent.commands)
    ...
    with open('/Users/bharathsurianarayanan/Desktop/gymjam/lander_angle_stats/best_fitness_{}.txt'.format(run_id), 'w') as filehandle:
    ...
    np.savetxt("/Users/bharathsurianarayanan/Desktop/gymjam/lander_angle_stats/successful_fitnesses"+current_run_id+".csv",successful_fitness_list, delimiter=",")
    np.savetxt("/Users/bharathsurianarayanan/Desktop/gymjam/lander_angle_stats/successful_seqeunces"+current_run_id+".csv",successful_sequence_list, delimiter=",")
